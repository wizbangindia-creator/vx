      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
              WhatsApp Template Name (BSP) *
            </label>
            <input
              type="text" value={form.wa_template_name}
              onChange={(e) => setForm({ ...form, wa_template_name: e.target.value })}
              placeholder="e.g. fair_reminder_3day"
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm font-mono"
              data-testid="wa-form-wa-name"
            />
            <p className="text-xs text-slate-400 mt-1">Must be an approved template in your WhatsApp provider</p>
          </div>
          <div>
        ...
        {isEdit && (
          <div className="border-t border-slate-200 pt-4 bg-slate-50 -mx-6 px-6 -mb-1 pb-4">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Test uses sample values (name=Test User, city=Jammu). Requires approved template at BSP.
            </p>
          </div>
        )}
